...
export default function ZoneSettingsPage() {
  ...
  const updateZone = (id: number | string, field: string, value: string) => {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      const valid = zones.filter((z) => z.zone_name.trim() && z.zip_text.trim());
      if (valid.length > 0) {
        const { error: insertErr } = await supabase.from("service_zones").insert(
          valid.map((z, i) => ({
            tenant_id: tenantUser.tenant_id,
            zone_name: z.zone_name.trim(),
            zip_codes: z.zip_text.split(",").map((s) => s.trim()).filter(Boolean),
            window_start: z.window_start,
            window_end: z.window_end,
            sort_order: i + 1,
            is_active: true,
      ...
      setSuccessMsg(`${valid.length} zone${valid.length !== 1 ? "s" : ""} saved`);
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <input
                    type="text"
                    value={zone.zip_text}
                    onChange={(e) => updateZone(zone.id, "zip_text", e.target.value)}
